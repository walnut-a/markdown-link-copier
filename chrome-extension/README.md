...
- 默认保留可能影响页面行为的通用参数，例如 `ref`、`source`、`from`、`campaign`。
- 使用页面标题和清理后的链接生成输出文本，并写入剪贴板。
- 优先读取页面里的文章标题、JSON-LD `headline`、`og:title`、`twitter:title`、`meta[name="title"]`、文章区域标题和 `h1`，拿不到时再回退到标签页标题。
- 页面开启翻译且缺少标题元数据时，会尝试从当前地址的原始 HTML 恢复未翻译的标题。
- 优先采用同站点的 canonical 地址，再清理其中的追踪参数。
- 弹出层会提示复制结果或错误信息。
- 复制成功后会显示实际复制内容与标题来源，方便核对识别结果。
- 清理过链接时会显示采用 canonical 地址、移除参数等处理摘要。
- 剪贴板写入失败时仍会保留生成结果，并只在失败状态显示再次复制入口。
- 可通过独立快捷键复制纯链接，同时去除全部查询参数和 `#` 片段。
- 弹出层提供 Chrome 快捷键设置入口，可分别配置链接文本与纯链接命令。
- 使用不打开弹窗的纯链接快捷键时，当前页面会短暂显示成功或失败反馈，并自动消失。
- 弹窗、设置页和页面反馈完整支持中文与英文；默认跟随浏览器，也可在设置页固定显示语言。
- 支持在设置页配置标题后缀清理规则，去掉站点 slogan。
- 支持在设置页配置 URL 清洗规则，包括删除参数、删除参数前缀和条件参数组。
- 内置 Markdown 链接、标题与链接分行、仅链接三种输出预设，也可以继续使用自定义模板。
- 支持导出设置、导入 JSON 设置文件和恢复默认设置。
...
8. 点击“快捷键”可进入 Chrome 扩展快捷键页面，分别为“复制当前页面链接文本”和“复制纯链接（移除所有参数）”分配按键组合。
9. 设置页顶部可选择“跟随浏览器”“中文”或“English”。扩展自己的界面会立即切换；`chrome://extensions/shortcuts` 属于 Chrome 自身页面，仍跟随浏览器语言。

## 规则配置

设置页中的规则会保存到 `chrome.storage.sync`，修改后立即影响下一次复制，不需要重新发版。

从旧版本升级时，仅当 URL 清洗配置仍完全等于旧版默认值，插件才会自动迁移到更安全的新默认值；手工修改过的配置保持不变。
...
选择内置输出预设时会自动更新输出模板；手工修改模板后，预设会显示为“自定义模板”。

## 备注

- 如果页面禁止访问剪贴板，需要再点击一次或允许相关权限。
- 对于纯前端渲染、需要特殊请求才能加载标题或禁止重新读取原始 HTML 的页面，无法保证恢复翻译前标题，此时会回退到当前页面标题。
- 绝大多数追踪参数和输出格式调整都可以直接在设置页完成。

## 发布打包

在仓库根目录运行 `./scripts/package-extension.sh`。生成的 `dist/markdown-link-copier-<版本号>.zip` 只包含扩展运行所需文件，不包含测试、README 和商店素材原图。
